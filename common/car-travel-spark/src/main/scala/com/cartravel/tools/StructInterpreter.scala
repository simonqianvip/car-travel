...
object StructInterpreter extends Logging {
   ...
   * @param conn
   */
  def interpreter(_tableName: String, parse: (String, Any), conn: Connection): Unit = {
    _tableName match {
      //订单表
      case tableName if (tableName.startsWith(order_info_table)) =>
        ...
        if (data.id != null) {
          ...
          HbaseTools.createTableBySelfBuilt(conn,TableName.valueOf(tableName),9,Array(columnFamily))
          //TODO 暂时只处理新增数据
          type_ match {
            case "insert" =>
              HbaseTools.putMapData(conn, tableName, 9, columnFamily, rowkey, mapData)
            case "update" =>
              HbaseTools.putMapData(conn, tableName, 9, columnFamily, rowkey, mapData)
            case _ => Nil
          }
