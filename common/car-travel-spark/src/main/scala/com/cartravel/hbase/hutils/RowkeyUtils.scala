...
object RowkeyUtils {
  ...
  //000X|+12wei
  def getRowkey(pk:String,regionNum:Int):String = {
    val prefix = md5SwithLong(pk,regionNum)
    //id_createTime
    //190817
    val split: Array[String] = pk.split("_")
    val rk = s"${split(0)}_${split(1).substring(2 , split(1).length)}"
    val suffix = MD5Hash.getMD5AsHex(Bytes.toBytes(rk)).substring(0 , 12)//转成16进制，并截取12位
    prefix + suffix
  }
  ...
  //获取 rowkey的前缀
  private def md5SwithLong(pk:String,regionNum:Int ):String = {
    val rkMd5 = MD5Hash.getMD5AsHex(Bytes.toBytes(pk)).substring(0 , 15)//转成16进制并截取15
    //0~9 a~f -->  0~15
    val hash: Long = md5ToLong(rkMd5)

    val str = (hash % regionNum).toString
