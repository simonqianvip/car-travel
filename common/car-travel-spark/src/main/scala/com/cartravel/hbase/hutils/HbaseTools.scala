...
object HbaseTools extends Logging{
  //建表
  def createTableBySelfBuilt(conn:Connection , tableName: TableName , regionNum:Int , column:Array[String]) = {
    this.synchronized{
      ...
      try{
        if(!admin.tableExists(tableName)){
          ...
          if(column !=  null){//如果列族不为空，则迭代
            column.foreach(c  => {
              val hcd = new HColumnDescriptor(c)//列描述
              hcd.setBlockCacheEnabled(false)
              hcd.setMaxVersions(1)//列最大版本数
              hcd.setBloomFilterType(BloomType.ROW)//行使用布隆过滤器
              hcd.setCompressionType(Algorithm.SNAPPY)//指定压缩算法
              tbDesc.addFamily(hcd)
            })
          }

          val splitKeysBuilder = new SplitKeysBuilder()
          val splitKeys: Array[Array[Byte]] = splitKeysBuilder.getSplitKeys(regionNum)
          admin.createTable(tbDesc , splitKeys)

        }
      }catch {
        case e: Exception => // 邮件报警
          print(s"异常：${e.printStackTrace()}")
      }
    }
  }
...
//    val admin = connection.getAdmin
    ...
    try{
      val rk = RowkeyUtils.getRowkey(key,9)
